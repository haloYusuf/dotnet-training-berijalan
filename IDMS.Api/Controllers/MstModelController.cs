...
namespace IDMS.Api.Controllers
{
    ...
    public class MstModelController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<IActionResult> GetList([FromQuery] ReqMstModelDto request)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] ReqMstModelCreateDto request)
        {
            var data = await _service.CreateAsync(request);
            return Ok(ApiResponseHelper.Success(HttpContext, data, "Model created successfully"));
        }

        [HttpPut("{id}")]
        public async Task<IActionResult> Update(int id, [FromBody] ReqMstModelUpdateDto request)
        {
            var data = await _service.UpdateAsync(id, request);
            return Ok(ApiResponseHelper.Success(HttpContext, data, "Model updated successfully"));
        }
        ...
        public async Task<IActionResult> SoftDelete(int id)
        {
            ...
            return Ok(ApiResponseHelper.Success(HttpContext, (object?)null, "Model deleted successfully"));
        }
    }
}